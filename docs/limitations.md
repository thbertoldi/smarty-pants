...
**Binary scope is Linux x86_64, glibc 2.35+.** The CPU bundle includes local inference and APIs; the smaller API bundle cannot switch to local inference. Vulkan/CUDA/ROCm require source builds. No Flatpak, container desktop image, signed RPM repository, or OBS publication is currently provided. SHA-256 files detect corruption; they are not independent publisher signatures.
