...
pub use smarty_pants_core::config::ChatTemplate as Template;
...
impl Prompt {
    pub fn new(system: &str, text: &str, language: Option<&str>) -> Self {
        Self {
            system: system.into(),
            text: text.into(),
    ...
    pub fn render(&self, template: Template) -> String {
        render(template, &self.system, &self.text, self.language.as_deref())
    }

    pub fn user_message(&self) -> String {
        format!(
            "{}<input>\n{}\n</input>{}",
            lang_pre(self.language.as_deref()),
            self.text.trim(),
            lang_post(self.language.as_deref())
        )
    }
}
...
pub fn render(template: Template, system: &str, user: &str, language: Option<&str>) -> String {
    match template {
...
fn lang_pre(language: Option<&str>) -> String {
    match language {
        Some(lang) => {
            format!("The input below is in {lang}. Your output MUST also be in {lang}.\n\n")
        }
        None => String::new(),
    }
}
