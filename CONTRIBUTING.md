...
For releases, update the workspace version, changelog, release notes, download examples and RPM spec together. Wait for CI, then push a matching `vX.Y.Z` tag. The [release workflow](.github/workflows/release.yml) builds on Ubuntu 22.04 with a generic x86_64 CPU target, tests both variants, packages dependency licenses and SHA-256 files, and tests installation before creating the GitHub release. `workflow_dispatch` builds downloadable workflow artifacts without publishing. See [RPM/OBS](packaging/rpm/README.md) for distribution packaging.
